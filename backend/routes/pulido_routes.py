...
from backend.models.sql_models import db, ProduccionPulido, PncPulido, BujeRevuelto, Producto, TrazabilidadLote, PulidoPendienteAutorizacion
...
def registrar_pulido():
    try:
        ...
        # Delegar la persistencia compleja al servicio
        res = PulidoService.ejecutar_persistencia_pulido(
...
def autorizar_pendiente_pulido():
    try:
        ...
        resultado = PulidoService.ejecutar_persistencia_pulido(
...
def pausar_pulido():
    ...
    id_pulido = data.get('id_pulido')
    try:
        registro = PulidoService.pausar(id_pulido, data.get('hora_pausa'))
        if not registro:
            return api_error("No encontrado", status_code=404)
        return api_success()
    except Exception as e:
        return api_error(str(e), status_code=500)
...
def reanudar_pulido():
    ...
    id_pulido = data.get('id_pulido')
    try:
        registro = PulidoService.reanudar(id_pulido, data.get('hora_reanudar'))
        if not registro:
            return api_error("No encontrado", status_code=404)
